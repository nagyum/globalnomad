...
export const getMyActivities = async (cursorId?: number, size: number = 20): Promise<MyActivitiesResponse> => {
  const response = await axiosClientHelper.get<MyActivitiesResponse>('/my-activities', { params: { cursorId, size } });
...
export const getReservations = async (
  ...
  const response = await axiosClientHelper.get<ReservationResponse>(`/my-activities/${activityId}/reservations`, {
    params,
  });

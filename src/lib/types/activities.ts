...
export const createActivitySchema = z.object({
  ...
  .max(4, { message: '소개 이미지는 최대 4개까지 등록할 수 있습니다.' }).optional(),
});
...
export const activityImageFormSchema = z.object({
  image: z
  .instanceof(File)
  .refine((file) => ['image/jpeg', 'image/jpg', 'image/png', 'image/ico', 'image/gif', 'image/webp'].includes(file.type), {

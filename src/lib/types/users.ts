...
export const userSchema = z.object({
  ...
  profileImageUrl: z
    .string()
    .url()
    .default('https://sprint-fe-project.s3.ap-northeast-2.amazonaws.com/globalnomad/profile_image/12-1_1757_1742022258900.png'),
...
export const profileImageUrlSchema = z
  .instanceof(File)
  .refine((file) => ['image/jpg', 'image/jpeg', 'image/png', 'image/ico', 'image/gif', 'image/webp'].includes(file.type), {
...
// 내 정보 수정 API 타입
export const userDataUpdateParams = z
  .object({
    nickname: z.string()
      .trim()
      .min(1, { message: '닉네임을 입력해주세요.'})
